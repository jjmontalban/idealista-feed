...
function idealista_render_admin_page() {
    ...
    <div class="wrap">
        ...
        if (isset($_GET['feed_status']) && $_GET['feed_status'] === 'customer_data_saved') {
            // Verificar el nonce
            if (!isset($_REQUEST['_wpnonce']) || !wp_verify_nonce($_REQUEST['_wpnonce'], 'idealista_store_customer_data')) {
                wp_die(esc_html__('Nonce verification failed.', 'idealista-properties-feed'));
            }

            if ($_GET['feed_status'] === 'customer_data_saved') {
            ...
            } elseif ($_GET['feed_status'] === 'success') {
                $file_name = 'properties_' . $form_values['code'] . '.json';
                $file_path = plugin_dir_path(__FILE__) . $file_name;
                $message = sprintf(__('JSON file generated and sent successfully', 'idealista-properties-feed'), $file_path);
                echo '<div id="message" class="updated"><p>' . esc_html($message) . '</p></div>';
            } elseif ($_GET['feed_status'] === 'ftp_missing') {
...
function idealista_store_customer_data() {
    ...
    // Generar un nuevo nonce para la próxima acción
    $new_nonce = wp_create_nonce('idealista_properties_feed_generate');
